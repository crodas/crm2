      ...
      <div className="card">
        ...
        {lines.map((line, idx) => (
          <div key={idx} className="card" style={{ background: 'var(--bg-app)' }}>
            ...
            <div className="mb-1">
              <label style={{ fontSize: '0.85rem', fontWeight: 500 }}>Prices by Customer Group</label>
              <div className="grid-2">
                {groups?.map((g: any) => {
                  const p = line.prices.find(p => p.customer_group_id === g.id)
                  return (
                    <div key={g.id} className="form-group">
                      <label>{g.name} ({g.default_markup_pct}% markup)</label>
                      <input
                        type="number"
        ...
        <div className="flex gap-1 mt-1">
          ...
          >
            {mutation.isPending ? 'Saving...' : 'Receive Inventory'}
          </button>
        </div>
